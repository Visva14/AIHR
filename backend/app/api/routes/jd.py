...
from app.schemas.job_description import JobDescriptionCreate, JobDescriptionRead, JobDescriptionUpdate
...
def deactivate_jd(
    ...
    if jd is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='JD not found')
    if jd.hr_user_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail='Not authorized to delete this JD')
